...
class HungarianAlgorithmAdvanced(matrix: Array<IntArray>) {
    ...
    // find row with marked zero
    fun findRowWithMarkedZero(): Int {
        for (i in rowMarkedZeros.indices) {
            if (rowMarkedZeros[i] != -1) {
                return i
            }
        }
        return -1
...
fun main() {
    ...
    optimal.print()
    println("Stupid employee:${hungarianAlgorithm.findRowWithMarkedZero()}")
}
